...
export function createEruptions(trafficPoints, globeRadius) {
  ...
  const mat = new THREE.ShaderMaterial({
    uniforms: {
      ...
      uPixelRatio: { value: Math.min(window.devicePixelRatio, 2) },
      uColor1: { value: new THREE.Color(0x4697f7) }, // severe (blue)
      uColor2: { value: new THREE.Color(0xf7a43e) }, // heavy (yellow/orange)
      uColor3: { value: new THREE.Color(0x61c16b) }, // moderate (green)
      uColor4: { value: new THREE.Color(0xf1666e) }, // light (red/pink)
    },
    vertexShader: /* glsl */ `
      uniform float uTime;
